...
integer :: i,j,i_t,i_proc,iproc,i_plot 
integer :: ios
integer :: tstep
...
do i_t=1,t_nstep
  ...
  do i_plot=1,nplot ! out processors
    ...
    bytes(3) = (plot_nelmt(i_plot))*size_int ! Offsets
    bytes(4) = (plot_nelmt(i_plot))*size_int ! Types
    bytes(5) = (out_ncomp*plot_nnode(i_plot))*size_float ! Nodal values        
    ...
    write(vtu_unit,'(a)')trim(buffer)
    write(num_str1,*)plot_nnode(i_plot); write(num_str2,*)plot_nelmt(i_plot); 
    ...
    if (i_t==1)then
      ...
      write(tmp_str,*)i_plot
      call open_file2write('./tmp/tmp_x_plot'//trim(adjustl(tmp_str))//char(0),fd_x)
      call open_file2write('./tmp/tmp_y_plot'//trim(adjustl(tmp_str))//char(0),fd_y)
      call open_file2write('./tmp/tmp_z_plot'//trim(adjustl(tmp_str))//char(0),fd_z)
      ! open temporary files to store connectivity list
      call open_file2write('./tmp/tmp_con_plot'//trim(adjustl(tmp_str))//char(0),fd_con)
      
      ! open mesh files and store mesh data
      tmp_nnode = 0
      elmt_count = 0
      node_count = 0
      do i_proc = 1, plot_nproc(i_plot)

        iproc = plot_proc_list(i_plot,i_proc)          
    ...
    call open_file2read('./tmp/tmp_y_plot'//trim(adjustl(tmp_str))//char(0),fd_y)
    call open_file2read('./tmp/tmp_z_plot'//trim(adjustl(tmp_str))//char(0),fd_z)
    ...
    call open_file2read('./tmp/tmp_con_plot'//trim(adjustl(tmp_str))//char(0),fd_con) 
    do i=1,plot_nelmt(i_plot)
      do j=1,NENOD_OUT
    ...
    if (out_ncomp>1)then ! vector or tensor data
      ...
      ! vector
        do i=1,plot_nnode(i_plot)
          do i_comp=1,out_ncomp
            call read_float(tmp_real,fd_array(i_plot))
      ...
      ! 9-component symmetric tensor
        do i=1,plot_nnode(i_plot)
          do i_comp=1,out_ncomp
            call read_float(tmp_rvect(i_comp),fd_array(i_plot))              
          enddo
...
do i_plot=1,nplot    
  write(tmp_str,*)i_plot
  call delete_file('./tmp/tmp_x_plot'//trim(adjustl(tmp_str))//char(0))
  call delete_file('./tmp/tmp_y_plot'//trim(adjustl(tmp_str))//char(0))
  call delete_file('./tmp/tmp_z_plot'//trim(adjustl(tmp_str))//char(0))
  call delete_file('./tmp/tmp_con_plot'//trim(adjustl(tmp_str))//char(0))
enddo
